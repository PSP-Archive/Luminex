 ...
 *
 * http://svn.ps2dev.org/filedetails.php?repname=psp&path=%2Ftrunk%2Flibpng%2Fscreenshot%2FMakefile&rev=0&sc=0
 *
...
void user_warning_fn2(png_structp png_ptr, png_const_charp warning_msg)
{
        // ignore PNG warnings
}
...
void showImage(const char* filename)
{
        ...
        png_set_error_fn(png_ptr, (png_voidp) NULL, (png_error_ptr) NULL, user_warning_fn2);
        info_ptr = png_create_info_struct(png_ptr);
        if (info_ptr == NULL) {
                fclose(fp);
